...
window.addEventListener('load', function() {
  ...
  var renderer = new quickJsProject.Renderer(screen_canvas);
  ...
  for (var i = 0; i < N; ++i) {
    path.curves[i] = new quickJsProject.Curve(i * 2, i * 2 + 1, null);  // Quadratic.

    var p1 = step();
    var p2 = step();
    path.points[i * 2 + 1] = quickJsProject.PathUtils.fitQuadraticToPoints(p0, p1, p2);
    path.points[i * 2] = p2;
    p0 = p2;
  }

  var colormap = [
    {n: 'r', c: new quickJsProject.RGBA(1, 0, 0, 1)},
    {n: 'g', c: new quickJsProject.RGBA(0, 1, 0, 1)},
    {n: 'b', c: new quickJsProject.RGBA(0, 0, 1, 1)},
    {n: 'a', c: new quickJsProject.RGBA(0, 1, 1, 1)},
    {n: 'y', c: new quickJsProject.RGBA(1, 1, 0, 1)},
    {n: 'w', c: new quickJsProject.RGBA(1, 1, 1, 1)}
  ];

  var fgcolor = new quickJsProject.RGBA(1, 0, 0, 1);
