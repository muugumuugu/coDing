...
window.addEventListener('load', function() {
  var black = new quickJsProject.RGBA(0, 0, 0, 1);
  var white = new quickJsProject.RGBA(1, 1, 1, 1);
  ...
  for (var i = 0; i < 10; ++i) {
    for (var j = 0; j < 10; ++j) {
      for (var k = 0; k < 10; ++k) {
        if (i == 0 || j == 0 || k == 0 ||
            i == 9 || j == 9 || k == 9) {
          var cube = quickJsProject.ShapeUtils.makeCube(0.5);
          var transform = new quickJsProject.Transform();
          transform.translate(i - 5, j - 5, k - 5);
          cubes.push({
            shape: cube,
            color: new quickJsProject.RGBA(i / 10, j / 10, k / 10, 0.3),
            trans: transform});
        }
      }
    }
