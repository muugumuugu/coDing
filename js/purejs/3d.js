...
function compose_n(xforms) {
  var rv = identity()
  for (var ii = 0; ii < xforms.length; ii++) rv = compose(rv, xforms[ii])
  return rv
}
//
...
// [points, lines, polies]
//each line is two indices into the points array
...
// given a shape, make a more complicated shape by copying it through transform xf n times, and connecting the corresponding points.  like a snail shell from a circle.
...
function make_torus(r1, r2, n1, n2) {
  ...
  extrude_shape(compose_n([transpose_axes(1, 2),
...
function draw_shape(canvas, xf, shape, alpha) {//most time heavy func :(
  ...
  var lines = shape[1]
  // 2/3 of the time is in this loop (when  not doing polies)
  if (alpha == null) {
  ...
  // when doing polies, 90% of our time is spent doing polies
  if (alpha != null) {
...
function draw_some_junk(canvas) {
  var xf = compose_n([transpose_axes(1, 2),
                     rotate(angle),
      	     	     transpose_axes(1, 2),
		     rotate(angle*1.618),
		     translate(0, 0, 2.5)])
...
function draw_torus(canvas) {
  ...
  draw_shape(canvas, compose_n([rotate(angle),
  		               transpose_axes(1, 2),
...
function update() {
  ...
  angle += 3.14159 / 90 // reduced from original π/30 because of frame rate boost
  cls($('canvas'))
  draw_torus($('canvas'))
}
function init(ev) {
...
/*
<html><head>
<meta http-equiv="content-type" content="text/html; charset=UTF-8"><title>3D-Rendered In Browser</title>
<script src="3d.js">
</script>
</head><body><h1>rotating torus</h1>

<p>
<!-- <input id="ms" /> ms -->
<input id="go" type="checkbox" checked="checked"><label for="go"> Go</label>
<input id="fill" type="checkbox" checked="checked"><label for="fill"> Solid</label>
<input id="translucent" type="checkbox" checked="checked"><label for="translucent"> Translucent</label>
<input id="trails" type="checkbox" checked="checked"><label for="trails"> Trails</label>
</p>
