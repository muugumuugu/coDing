...
function plant(size, angle)
{
    ...
    delta = ps - ps * .618
    pencolor("black");
    forward_taper(size, ps, ps - delta * 1/3);
  
    left(angle);
      plant(size * .618, angle);
    right(angle);
    
    ps = size * .3;
    delta = ps - ps * .618
    pencolor("black");
    right(5); forward_taper(size, ps - delta * 1/3, ps - delta * 2/3);
    ...
    delta = ps - ps * .618
    pencolor("black");
    right (3); forward_taper(size, ps - delta * 2/3, ps - delta * 3/3);
      plant(size * .618, angle);

    //penup();
    back(size);left(3);back(size);left(5);back(size)
}

function forward_taper(size, start_ps, end_ps)
{
  s = size/3;
  pendown();
  ...
  pensize(start_ps - (start_ps - end_ps)/2);
  forward(s);
