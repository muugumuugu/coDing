...
Dim x2j_1 As Double, x2j As Double, x2jaa As Double, x2j_1aa As Double
Dim j As Long, N1 As Long
Dim h As Double, Int1 As Double, Int2 As Double
Dim sum1 As Double, sum2 As Double
Dim prog As Double
Dim over As Double, under As Double
Dim cstrcoordformat
...
N1 = Round(Val(Text3.Text), 0)
If N1 Mod 2 = 1 Then N1 = N1 + 1
Text3.Text = Trim(Str(N1))
h = (xN - x0) / N1 'Distance between two points

    xN_1 = x0 + (N1 - 1) * h 'Calculate x[N-1]
    Int1 = GetVal(x0, 0, 0, 0) + 4 * GetVal(xN_1, 0, 0, 0) + GetVal(xN, 0, 0, 0) 'Calculate f(x0)+4*f(x[N-1])+f(xN)
    Int2 = Abs(GetVal(x0, 0, 0, 0)) + 4 * Abs(GetVal(xN_1, 0, 0, 0)) + Abs(GetVal(xN, 0, 0, 0)) 'Calculate f(x0)+4*f(x[N-1])+f(xN)
    ...
    For j = 1 To (N1 / 2 - 1)
        ...
        x2j_1aa = GetVal(x2j_1, 0, 0, 0)
        x2jaa = GetVal(x2j, 0, 0, 0)
