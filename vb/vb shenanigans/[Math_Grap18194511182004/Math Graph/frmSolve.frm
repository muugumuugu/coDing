...
If x > searchstart Then
    ...
    If Round(GetVal(Round(Root, 6), 0), 6) <> 0 Then Error = True
    ...
    slope1 = SlopeCalc(lastx - thestep)
    slope2 = SlopeCalc(x)
   
    Root = Round(touchroot(lastx - thestep, x, slope1, slope2), 4)
        If Round(GetVal(Round(Root, 6), 0), 5) <> 0 Then Error = True
        If math.Error = True Then Error = True
        If Error = False Then
        List1.AddItem Format(Str(Round(Root, 6)), cstrcordformat)
        rootsnum = rootsnum + 1
        End If
        math.Error = False
    End If
...
If rootsnum = 0 Then
Label4.Caption = "no roots found, select another interval."
Else
...
Else
Dim guess As Double, ypoint As Double
tries = 0
...
Loop
math.Error = False
test = GetVal(Round(guess, 6), 0)
If tries < 200 And math.Error = False Then
List1.AddItem Trim(Format(Str(Round(guess, 6)), cstrcoodrformat))
